...
class BootStrap {
    ...
    def init = { servletContext ->
        ...
        /**  Reading properties from external config app-config.yml (Ques-13) //Introduction to grails
        Map config = ApplicationContextHolder.config.get("dataSource.username")
        log.info(config)
         */
    }
    List<User> createUser() {
        ...
        User normalUser = new User(userName: "user", firstName: "ankit", lastName: "Aggarwal", email: "user@ttn.com",
                password: Constants.PASSWORD, admin: false, active: false)
        User adminUser = new User(userName: "admin", firstName: "priya", lastName: "sharma", email: "admin@ttnd.com",
